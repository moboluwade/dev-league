...
const Signup = () => {
  ...
  useEffect(() => {
    console.log("current auth Count is this: ", currentFlow);
  }, [currentFlow]);
  ...
  const emailVerification = useMutation({
    ...
    mutationFn: async ({ email }) => {
      const res = axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/admin/validate-email`,
        { email: email },
  ...
  const passcodeVerification = useMutation({
    ...
    mutationFn: async ({ passcode }) => {
      const res = axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/admin/validate-passcode`,
        { passcode: passcode },
        { withCredentials: "include" }
      );
      return res;
  ...
  useEffect(() => {
    ...
    signupMutation.error &&
      console.log(signupMutation.status, signupMutation.error);
  }, [
    navigate,
  ...
  const handleSignup = async (event) => {
    ...
    if (confirmPassword === password) {
      if (currentFlow === 2) {
        try {
          const feedback = await signupMutation.mutateAsync({
            email: email,
            password: password,
          });
