...
export default function EditBlog() {
  ...
  const updateBlog = useMutation({
    mutationFn: (updatedBlog) => {
      return axios.patch(`${import.meta.env.VITE_BACKEND_URL}/api/blog/update/${blogId}`, updatedBlog)
    },
  })
  ...
  return (
    <div className="relative w-full max-h-screen overflow-y-auto bg-text-dev-light-orange">
      <div className="max-w-4xl p-6 mx-auto space-y-8 md:mx-0">
        {showSuccessModal && (
        ...
        <div className="space-y-6">
            ...
            {fileAdded ? (
              <div className="mt-4 rounded-lg">
                <img
                  src={fileAdded}
                  alt="Preview"
                  className="object-cover rounded-md shadow-md h-44 aspect-video"
                />
              </div>
            ) : (
          ...
          <div>
            <label className="block mb-2 text-sm font-bold">Blog Type</label>
            <div className="flex flex-wrap gap-2 p-4 border-[1.5px] border-[#292422] w-fit md:min-w-[28rem] rounded-md bg-white">
              {["articles", "news", "tech", "jobs"].map((type) => (
                <div
                  key={type}
                  className="flex items-center mx-auto space-x-2 select-none"
                >
                  <label
                  ...
                  >
                    <input
                      type="checkbox"
                      className="hidden w-4 h-4 rounded-full form-checkbox text-text-dev-orange"
                      value={type}
