...
const Articles = () => {
  ...
  const SliceText = (text) => {
    ...
    if (length > 65) {
      const slicedText = length.slice(0, 65)
  ...
  return (
    <div className="flex flex-col items-center w-full pb-12 text-center text-black h-fit">
      <div className='lg:px-[7.5rem] md:px-12 px-4 max-w-[87.5rem] flex flex-col justify-center items-center'>
        ...
        <div className="flex flex-wrap items-start justify-center gap-8 pb-12 text-black">
          {Cards.map((card) => {
            const { id, title, desc, img, person } = card
            return (
              <div key={id} className="flex flex-col border border-[#D4CECB] lg:max-w-[19rem] max-w-[18rem] bg-white rounded-lg text-start">
                <div>
